(function (root, factory) {
  ...
  // Values verified against the stock competitive configuration on the reference
  // CS2 installation. They are context, not promises: game-mode CFGs and later
  // exec files can override them.
  const COMPETITIVE_BASELINES = {
    warmup_time: 120, freeze_time: 15, max_rounds: 24, round_time: 115.2,
    halftime: true, clinch: true, friendly_fire: true, solid_teammates: 1,
    respawn_ct: false, respawn_t: false, buy_time: 20, buy_anywhere: false,
    start_money: 800, max_money: 16000, player_cash_awards: true,
    team_cash_awards: true, free_armor: 0, ct_primary: "", t_primary: "",
    ct_secondary: "weapon_hkp2000", t_secondary: "weapon_glock", melee: true,
    infinite_ammo: 0, map_weapons: true, drop_guns: 1, bot_quota: 1,
    bot_quota_mode: "competitive", bot_difficulty: 2, bot_chatter: "normal",
    dead_talk: true, enemy_living_talk: false, enemy_dead_talk: false,
    force_camera: 1,
  };
  ...
  function newConfig(presetId = "research_1v1") {
    const config = { schema_version: 1, kind: "counterform-match-config", config_name: `${presetId}_match`, preset: presetId, settings: {}, unmanaged_lines: [], import_warnings: [] };
  ...
  function normalizeConfig(input) {
    ...
    const config = { schema_version: 1, kind: "counterform-match-config", config_name: slug(input.config_name), preset: PRESETS[input.preset] ? input.preset : "custom", settings: {}, unmanaged_lines: Array.isArray(input.unmanaged_lines) ? input.unmanaged_lines.map(String) : [], import_warnings: Array.isArray(input.import_warnings) ? input.import_warnings.map(String) : [] };
  ...
  function exportCfg(config) {
    ...
    const lines = ["// Generated by Counterform Match Config", `// counterform-config-name: ${config.config_name}`, "// Persistent managed settings; execute after the active game-mode CFG.", "// BEGIN COUNTERFORM MANAGED SETTINGS", ""];
    ...
    lines.push("// END COUNTERFORM MANAGED SETTINGS");
    const unmanaged = Array.isArray(config.unmanaged_lines) ? config.unmanaged_lines : [];
    if (unmanaged.length) {
      lines.push("", "// BEGIN PRESERVED UNMANAGED LINES", ...unmanaged, "// END PRESERVED UNMANAGED LINES");
    }
    return `${lines.join("\n").trimEnd()}\n`;
  }

  function unquoteCfgToken(token) {
    if (!token.startsWith('"')) return token;
    return token.slice(1, -1).replace(/\\"/g, '"').replace(/\\\\/g, "\\");
  }

  function decodeValue(entry, encoded) {
    let value = encoded;
    if (entry.encode === "minutes") value = Number(encoded) * 60;
    else if (entry.encode === "knife") value = String(encoded) !== "";
    else if (entry.type === "boolean") {
      if (!/^[01]$/.test(String(encoded))) throw new Error("expected 0 or 1");
      value = String(encoded) === "1";
    } else if (entry.type === "number") {
      value = Number(encoded);
      if (!Number.isFinite(value)) throw new Error("expected a number");
    }
    if (entry.type === "number" && (value < entry.min || value > entry.max)) throw new Error(`expected ${entry.min} to ${entry.max}`);
    if (entry.type === "select" && !(entry.options || []).some(([candidate]) => String(candidate) === String(value))) throw new Error("unsupported option");
    return normalizeValue(entry, value);
  }

  function parseCfg(text, filename = "counterform_match.cfg") {
    const config = newConfig("blank");
    config.config_name = slug(String(filename).replace(/\.cfg$/i, ""));
    config.preset = "custom";
    config.unmanaged_lines = [];
    config.import_warnings = [];
    const byCvar = new Map();
    REGISTRY.forEach((entry) => entry.cvars.forEach((cvar) => byCvar.set(cvar.toLowerCase(), entry)));
    const seen = new Map(); const seenCvars = new Set(); const entryValues = new Map();
    let inManagedBlock = false;
    let inPreservedBlock = false;
    String(text || "").replace(/\r\n/g, "\n").split("\n").forEach((line, index) => {
      const trimmed = line.trim();
      const nameMatch = trimmed.match(/^\/\/\s*counterform-config-name:\s*(.+)$/i);
      if (nameMatch) { config.config_name = slug(nameMatch[1]); return; }
      if (/^\/\/\s*BEGIN COUNTERFORM MANAGED SETTINGS$/i.test(trimmed)) { inManagedBlock = true; return; }
      if (/^\/\/\s*END COUNTERFORM MANAGED SETTINGS$/i.test(trimmed)) { inManagedBlock = false; return; }
      if (/^\/\/\s*BEGIN PRESERVED UNMANAGED LINES$/i.test(trimmed)) { inPreservedBlock = true; return; }
      if (/^\/\/\s*END PRESERVED UNMANAGED LINES$/i.test(trimmed)) { inPreservedBlock = false; return; }
      if (/^\/\/\s*(Generated by Counterform Match Config|Persistent managed settings)/i.test(trimmed)) return;
      if (inPreservedBlock) { config.unmanaged_lines.push(line); return; }
      if (!trimmed || trimmed.startsWith("//")) {
        if (!inManagedBlock && trimmed) config.unmanaged_lines.push(line);
        return;
      }
      const match = line.match(/^\s*([A-Za-z0-9_]+)\s+("(?:\\.|[^"\\])*"|[^\s;]+)\s*(?:\/\/.*)?$/);
      if (!match) { config.unmanaged_lines.push(line); return; }
      const cvar = match[1].toLowerCase(); const entry = byCvar.get(cvar);
      if (!entry) { config.unmanaged_lines.push(line); return; }
      try {
        const value = decodeValue(entry, unquoteCfgToken(match[2]));
        if (seenCvars.has(cvar)) config.import_warnings.push(`Line ${index + 1}: ${match[1]} appeared more than once; the last supported value is shown.`);
        seenCvars.add(cvar);
        const priorValues = entryValues.get(entry.id) || [];
        if (priorValues.length && !priorValues.some((prior) => String(prior) === String(value))) config.import_warnings.push(`Line ${index + 1}: ${entry.label}'s related cvars disagree; the last supported value is shown and export will synchronize them.`);
        entryValues.set(entry.id, [...priorValues, value]);
        seen.set(entry.id, { value, cvar });
        config.settings[entry.id] = { enabled: true, value };
      } catch (error) {
        config.unmanaged_lines.push(line);
        config.import_warnings.push(`Line ${index + 1}: ${entry.label} was preserved as unmanaged (${error.message}).`);
      }
    });
    REGISTRY.filter((entry) => entry.cvars.length > 1 && seen.has(entry.id)).forEach((entry) => {
      const present = entry.cvars.filter((cvar) => new RegExp(`^\\s*${cvar}\\s+`, "im").test(text));
      if (present.length !== entry.cvars.length) config.import_warnings.push(`${entry.label}: only ${present.length} of ${entry.cvars.length} team cvars was present; export will write both.`);
    });
    return config;
  }

  function valueLabel(entry, value) {
    if (entry.type === "boolean") return value ? "On" : "Off";
    if (entry.type === "select") return optionLabel(entry, value);
    return `${formatNumber(Number(value))}${entry.unit ? ` ${entry.unit}` : ""}`;
  }

  function settingExplanation(entry, state) {
    const selected = valueLabel(entry, state.value);
    const cvars = entry.cvars.join(entry.cvars.length > 1 ? " and " : "");
    const baseline = Object.prototype.hasOwnProperty.call(COMPETITIVE_BASELINES, entry.id)
      ? `Reference competitive CFG: ${valueLabel(entry, COMPETITIVE_BASELINES[entry.id])}. Game modes and later CFGs may override it.`
      : "No single competitive baseline is shown because the stock game-mode files do not define a reliable value for this control.";
    return { selected: `${selected}: ${entry.description}`, technical: `Counterform writes ${cvars} only while Include is checked.`, baseline };
  }
  ...
  function matchPreview(config) {
    ...
    return {
      ...
      warnings: [...report.warnings, ...(config.import_warnings || [])],
      unmanagedCount: (config.unmanaged_lines || []).filter((line) => line.trim()).length,
      enabledCount: Object.values(config.settings || {}).filter((item) => item?.enabled).length,
    };
  }

  return { CATEGORIES, REGISTRY, PRESETS, COMPETITIVE_BASELINES, clone, slug, registryById, newConfig, applyPreset, normalizeConfig, normalizeValue, settingState, setSetting, validateConfig, exportCfg, parseCfg, searchRegistry, optionLabel, valueLabel, settingExplanation, formatDuration, matchPreview };
});
